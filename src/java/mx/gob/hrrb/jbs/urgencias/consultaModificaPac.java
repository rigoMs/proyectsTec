...
import mx.gob.hrrb.jbs.hospitalizacion.hojaCODE;
...
@ManagedBean(name = "oConsModPac")
@SessionScoped
//@ViewScoped

public class consultaModificaPac{
    ...
    private boolean bBandNiv=false;
    private boolean bBandFam=false;
    ...
    public consultaModificaPac(){
        ...
        oPerfil = new Perfil();
        try {
            bPerfil = oPerfil.buscaPerfilUrgs();
        } catch (Exception ex) {
    ...
    return lTipoAsentamiento;
     ...
     //Retorna Lista de Claves de Edad
     public List<Parametrizacion> getClaveEdad(){
        List<Parametrizacion> lLista = null;
       try {
           lLista = new ArrayList<Parametrizacion>(Arrays.asList(
                   (new Parametrizacion()).buscarClaveEdad()));
       } catch (Exception ex) {
           Logger.getLogger(registrarPaciente.class.getName()).log(Level.SEVERE, null, ex);
       }
        return lLista;
    ...
    public boolean habilitaNumSeg(String opc){
        ...
        boolean hab=true;
            ...
            if (getPacCMUrg().getPaciente().getSeg().getUnaDer().compareTo(opc)==0){
                hab=false;
            }
    ...
    public boolean habilitaPais(){
        ...
        boolean p=false;
        p = !(oAdmisionUrgs.getPaciente().getPais().compareToIgnoreCase("MÉXICO")==0 || oAdmisionUrgs.getPaciente().getPais().compareToIgnoreCase("MEXICO")==0);
        return p;
    }
    
    public boolean habilitaTelefono(){
    boolean bRet = false;
    if(oAdmisionUrgs.getFamiliarCercano().getNoTel1()!=null){
        bRet = oAdmisionUrgs.getFamiliarCercano().getNoTel1().compareTo("") != 0;
    }
        return bRet;
    }
    
    public String habilitaDatosHosp(){
        if(oAdmisionUrgs.getStatusPac().compareTo("HOSPITALIZADO")==0)
            return "true";
        else 
            return "false";
    }
    ...
    public boolean habilitaDir(){
        boolean p=true;
        p = oAdmisionUrgs.getPaciente().getPais().compareToIgnoreCase("MÉXICO")==0 || 
                oAdmisionUrgs.getPaciente().getPais().compareToIgnoreCase("MEXICO")==0;
        return p;
    }
    
     public boolean habilitaLengua(){
        boolean b=false;
        b = oAdmisionUrgs.getPaciente().getEtnicidad().getHablaLenguaIndStr().compareToIgnoreCase("T0201") != 0;
        return b;
    }
    ...
    public AdmisionUrgs getPacCMUrg(){
        if (!bBuscado){
            bBuscado = true;
            if (this.sOpe.equals("m")){
                try{              
                    ...
                    getAdmisionUrgs().buscarPacienteCMUrg();
                }catch(Exception e){
                    e.printStackTrace();
                    bBuscado = false;
                    FacesMessage message = new FacesMessage(
                            ...
                            "Modificar Paciente", 
                            "No se Puede Modificar al paciente debido a que nunca ha ingresado al área de urgencias");
    ...
    public String habilitaBotonM(){
        if(getOpe().compareTo("m")==0){
            getPacCMUrg().setDig1(getAdmisionUrgs().getFolio());
            getPacCMUrg().setFechaAdm(getAdmisionUrgs().getFecha().getTime().toLocaleString());}
        if(getPacCMUrg().getPaciente().getNombres().compareTo("")==0)
            return "mostrarbtn";
        else if(getOpe().compareTo("m")==0)
            return "";
        else return "mostrarbtn";
    }
    ...
    public boolean activaVigenciaNumero1(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("01")==0;
    }
    public boolean activaVigenciaNumero2(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("02")==0;
    }
    
    public boolean activaVigenciaNumero3(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("03")==0;
    }
    
    public boolean activaVigenciaNumero4(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("04")==0;
    }
    
    public boolean activaVigenciaNumero5(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("05")==0;
    }
    
    public boolean activaVigenciaNumero6(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("06")==0;
    }
    
    public boolean activaVigenciaNumero7(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("07")==0;
    }
    
    public boolean activaVigenciaNumero8(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("08")==0;
    }
    
    public boolean activaVigenciaNumeroG(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("0G")==0;
    }
    
    public boolean activaVigenciaNumeroP(){
        return getAdmisionUrgs().getPaciente().getSeg().getUnaDer().compareTo("0P")==0;
    }
    
    public String fechaNivel(){
        if (getAdmisionUrgs().getNivelSocioEcoStr().compareTo("")==0)
            return "false";
        else 
            return "true";
    }
    
    public String activaFamiliar(){
        if (getAdmisionUrgs().getFamiliarCercano().getParentescoStr()==null  || getAdmisionUrgs().getFamiliarCercano().getParentescoStr().compareTo("")==0)
            return "false";
        else 
            return "true";
    }
    
    public String habilitarCampoFolio(){
        if (bPerfil==false)
                return "true";
            else 
                return "false";
    ...
    public boolean getBandNiv() {
        return bBandNiv;
    }

    public void setBandNiv(boolean bBandNiv) {
        this.bBandNiv = bBandNiv;
    }

    public boolean getBandFam() {
        return bBandFam;
    }

    public void setBandFam(boolean bBandFam) {
        this.bBandFam = bBandFam;
    }
    //Retorna lista de niveles SocioEconomicos

    public List<Parametrizacion> getListaNivelesSocEco(){
        List<Parametrizacion> lLista = null;
       try {
           lLista = new ArrayList<Parametrizacion>(Arrays.asList(
                   (new Parametrizacion()).buscarTabla("T36")));
       } catch (Exception ex) {
           Logger.getLogger(consultaModificaPac.class.getName()).log(Level.SEVERE, null, ex);
       }
        return lLista;
       ...
       //Retorna lista de Estados de Salud
   public List<Parametrizacion> getListaEdosSalud(){
        List<Parametrizacion> lLista = null;
       try {
           lLista = new ArrayList<Parametrizacion>(Arrays.asList(
    ...
    public String modificaPacCM() throws Exception{
        ...
        if (encontradoAux.compareTo("")==0){
            afec=oAdmisionUrgs.modificarPacUrgCM();
            if(afec==0){
                message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Modificación de Paciente", "Modificación fallida!!!");
                pag="/ConsultaModificaPac.xhtml";
            }else{
                message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Modificación de Paciente", "Modificación exitosa!!!");
            }
        }else{
            message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Modificación de Paciente", "El número de seguro popular ya existe para el paciente: "+encontradoAux);
            pag="/ConsultaModificaPac.xhtml";
        }
        RequestContext.getCurrentInstance().showMessageInDialog(message);
        return pag;
